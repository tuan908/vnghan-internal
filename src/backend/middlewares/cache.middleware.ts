...
// Use a connection pool approach for serverless
const cacheStoreConnections = new Map<string, RedisCacheStore>();
...
export const createCacheMiddleware = (
	...
	return async (c: Context, next: Next) => {
		...
		const actualRedisUrl = REDIS_URL || opts.REDIS_URL;
		const actualRedisToken = REDIS_TOKEN || opts.REDIS_TOKEN;
		const user = c.get("user");

		// Check if Redis is configured
		if (!actualRedisUrl || !actualRedisToken) {
		...
		const connectionKey = `${actualRedisUrl}:${opts.namespace || "default"}`;

		try {
			// Get or create cache store for this connection
			if (!cacheStoreConnections.has(connectionKey)) {
				cacheStore = new RedisCacheStore(
					actualRedisUrl,
					actualRedisToken,
					opts.namespace,
				);
				cacheStoreConnections.set(connectionKey, cacheStore);
			} else {
				cacheStore = cacheStoreConnections.get(connectionKey)!;

				// Check if connection is healthy
				const isConnected = await cacheStore.ping().catch(() => false);
				if (!isConnected) {
			...
			if (!opts.methods?.includes(c.req.method)) {
				await invalidateCache(cacheStore);
				return await next();
			}
			...
			try {
				cachedEntry = await cacheStore.get(cacheKey);
			} catch (error) {
				...
				if (cachedEntry.isFile && cachedEntry.fileData) {
					...
					if (opts.compressFiles && cachedEntry.compressed) {
						try {
							...
							const inflate = util.promisify<Buffer, Buffer>(
								(input, callback) => zlib.inflate(input, callback),
							);
			...
			// Clean up any unnecessary connections (helps with serverless deployment)
			if (Math.random() < 0.01) {
				// 1% chance to run cleanup to avoid doing it on every request
				try {
			...
			// If Redis fails completely, just continue without caching
