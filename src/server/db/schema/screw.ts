import { index, integer, jsonb, serial, text } from "drizzle-orm/pg-core";
import { pgTableWithAudit } from "../base/helpers";

export const screwMaterial = pgTableWithAudit(
	"screw_material",
	{
	...
	(table) => [index("idx_screw_material_name").on(table.name)],
);
...
export const screwType = pgTableWithAudit(
	"screw_type",
	{
		id: serial("id").primaryKey(),
		name: text("name"),
	...
	(table) => [index("idx_screw_type_name").on(table.name)],
);
...
export const screw = pgTableWithAudit(
	"screw",
	{
		id: serial("id").primaryKey(),
		name: text("name"),
	...
	(table) => [
		index("idx_screw_material_type").on(
			table.materialId,
